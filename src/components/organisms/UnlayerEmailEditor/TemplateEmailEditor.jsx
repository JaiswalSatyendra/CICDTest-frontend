...
const TemplateEmailEditor = () => {
  ...
  useEffect(() => {
    setS3Location(localStorage.getItem("s3Location"));
  ...
  const getEmailTemplateJSON = async () => {
    await axios
      .post(`${process.env.REACT_APP_API_URL}/s3/read`, {
        bucket: "convertml-static-email-templates",
        key:
          s3Location.replace(/\+/g, " ").split("/").slice(3).join("/") +
          "/index.json",
      })
      .then((data) => {
