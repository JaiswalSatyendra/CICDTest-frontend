...
const Results = ({ datasets, fetchDatasets, getDataAnalysis, getDataPredict }) => {
  ...
  useEffect(async () => {
    setGridDataForGrid({
      columns: new Array(
        { field: "project_id", headerName: "ProjectId", sortable: true, width: 250, },
        { field: "model_name", headerName: "Model Name", sortable: true, width: 160, },
        { field: "table_name", headerName: "Dataset Name", sortable: true, width: 200, },
        { field: "created_on", headerName: "Started Time", sortable: true, width: 190, },
        {
        ...
        {
          field: "status", headerName: "Status", sortable: true, width: 170, renderCell: (data) => {
            return (<div>
              <Box display="flex" justifyContent="flex-start">
  ...
  const [toggleView1, setToggleView1] = useState("trained_view");

  const handleViewOrientation = (_event, newValue) => {
    setToggleView(newValue);
  };

  const handleViewOrientation1 = (_event, newValue) => {
    if (newValue === "trained_view") {
      getDataAnalysis()
      setToggleView1("trained_view");
    } else {
      getDataPredict()
      setToggleView1("prediction_view");
    }
  };
          ...
          <Grid item xs={12}
            ...
            <Box p={1}>
              <TextField
                ...
                placeholder={t("Search by project id...")}
                value={query}
                fullWidth
                variant="outlined"
              />
          ...
          >
            <Box p={1}>
              <ToggleButtonGroup
                value={toggleView1}
                exclusive
                onChange={handleViewOrientation1}
              >
                <ToggleButton disableRipple value="trained_view">
                  <Tooltip title={t("Trained")} arrow>
                    <ArticleIcon fontSize="small" />
                  </Tooltip>
                </ToggleButton>
                <ToggleButton disableRipple value="prediction_view">
                  <Tooltip title={t("Prediction")} arrow>
            ...
            <Box p={1}>
              ...
              >
                ...
                <ToggleButton disableRipple value="grid_view">
                  <Tooltip title={t("Grid view")} arrow>
                    <GridViewTwoToneIcon />
                  </Tooltip>
                </ToggleButton>
              </ToggleButtonGroup>
          ...
          {!selectedBulkActions && (
            ...
            >
              <Box>
                ...
                <b>{paginatedDatasets.length}</b> <b>{t("models")}</b> */}
                <Tooltip title={t("Refresh grid")} arrow>
                  <IconButton
                    onClick={() => {
                      toggleView1 === "trained_view" ? fetchDatasets() : getDataPredict()
                    }}
                    color="primary"
                  >
                ...
                <Table>
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell>{t("Dataset Name")}</TableCell>
                    ...
                    {paginatedDatasets.map((dataset) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell>
                            <Typography noWrap variant="h5">
                              {toggleView1==='trained_view'?dataset.table_name:dataset.predict_data_source}
                            </Typography>
                          </TableCell>
                          <TableCell>
            ...
            <>
              <Grid container spacing={3}>
                {paginatedDatasets.map((dataset) => {
                  ...
                  return (
                    <Grid item xs={12} sm={6} md={4} key={dataset.projectid}>
                      <CardWrapper
                        className={clsx({
